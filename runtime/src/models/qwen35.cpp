...
bool Qwen35Model::batched_forward(const int* token_ids, int n, int start_pos, bool /*resume_gdn*/,
    ...
    // Row-batched verify first: one weight stream for the whole window instead of one per
    // token. dflash_verify_rows rejects unsupported configs / window sizes before touching
    // any device state, so the token-loop fallback below never sees a half-advanced KV or
    // GDN state. Rollback of rejected tokens stays the caller's job on both paths.
    set_dflash_capture_row(0);   // window rows capture into dflash_hidden[0..n)
    if (dflash_verify_rows(token_ids, n, start_pos, out_argmax))
        return true;
...
std::vector<int> Qwen35Model::dflash_generate(const std::vector<int>& prompt, int max_new,
    ...
    // Verify the proposal tail as one row-batched window. SPARKINFER_DFLASH_ROWS=0 restores the
    // token loop, which is the A/B control and the fallback if a config is unsupported.
    ...
    while ((int)out.size() < max_new) {
        ...
        if (!vfail && block[1] == p0) {
            ...
            if (rows_verify && kProposalDepth >= 2) {
                // Snapshot the window head in place on the decode stream. save_spec_snapshot()
                // would do the same two copies but also block the host on them, and nothing
                // here needs the host: the copies are already ordered ahead of the window.
                const size_t ls = (size_t)s.cfg.n_layers * s.cfg.linear_v_heads *
                                  s.cfg.linear_head_dim * s.cfg.linear_head_dim;
                const size_t cs = (size_t)s.cfg.n_layers * (s.cfg.linear_conv_kernel - 1) * s.linear_qkvdim;
